...
class ASCToCSVApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        """
        self.root = root
        self.root.title("ASC to CSV 转换工具 v1.1.0")
    ...
    def _validate_inputs(self) -> bool:
        ...
        Returns:
            bool: 验证是否通过
        """
        if not self.asc_entry.get():
            messagebox.showerror("错误", "请选择ASC文件")
            return False
        
        if not os.path.exists(self.asc_entry.get()):
            messagebox.showerror("错误", "ASC文件不存在")
            return False
        ...
        try:
            float(self.sample_interval_var.get())
        except ValueError:
            messagebox.showerror("错误", "采样间隔必须是数字")
            return False
        
        try:
            int(self.group_size_var.get())
        except ValueError:
            messagebox.showerror("错误", "分组大小必须是整数")
            return False
